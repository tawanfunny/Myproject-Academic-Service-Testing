...
Resource    TC11_EditProposal.robot


*** Keywords ***
Go To Academic_Services
    [Arguments]    ${i}
    Open Excel Document    ${datatable}    TC11-EC
...
Fill Proposal Form
    ...
    ${ProjectTitle}    Read Excel Cell    ${i}    4
        ...
        Run Keyword And Return Status    Input Text    //div[@id='projectPeriod']//p   ${DurationOfOperation}

        ${Budget}    Read Excel Cell    ${i}    10
        ${Budget}=    Evaluate    '' if $Budget in ['None', '', None] else $Budget
        Run Keyword And Return Status    Input Text    //div[@id='budget']//p   ${Budget}
        
        Click Element   //button[contains(text(),'เพิ่มแถว')]

        ${StudentId}    Read Excel Cell    ${i}    11
        ${StudentId}=    Evaluate    '' if $StudentId in ['None', '', None] else $StudentId
        Run Keyword And Return Status    Input Text    //input[@name='memberStudentId[]']    ${StudentId}

        ${StudentFName}    Read Excel Cell    ${i}    12
        ${StudentFName}=    Evaluate    '' if $StudentFName in ['None', '', None] else $StudentFName
        Run Keyword And Return Status    Input Text    //input[@name='memberFName[]']    ${StudentFName}

        ${StudentLName}    Read Excel Cell    ${i}    13
        ${StudentLName}=    Evaluate    '' if $StudentLName in ['None', '', None] else $StudentLName
        Run Keyword And Return Status    Input Text    //input[@name='memberLName[]']    ${StudentLName}

        ${Position}    Read Excel Cell    ${i}    14
        ${Position}=    Evaluate    '' if $Position in ['None', '', None] else $Position
        Run Keyword And Return Status    Input Text    //input[@name='memberPosition[]']    ${Position}

        ${Location}    Read Excel Cell    ${i}    15
        ${Location}=    Evaluate    '' if $Location in ['None', '', None] else $Location
        Run Keyword And Return Status    Input Text    css:#location    ${Location}

        ${RiskAndSolve}    Read Excel Cell    ${i}    16
        ${RiskAndSolve}=    Evaluate    '' if $RiskAndSolve in ['None', '', None] else $RiskAndSolve
        Run Keyword And Return Status    Input Text    //div[@id='riskAndSolve']//div[@class='ql-editor']    ${RiskAndSolve}
        ...
        Run Keyword And Return Status    Input Text    //div[@id='evaluation']//div[@class='ql-editor']    ${EvaluationCriteria}

        ${ProjectAdvisor}    Read Excel Cell    ${i}    18
        ${ProjectAdvisor}=    Evaluate    '' if $ProjectAdvisor in ['None', '', None] else $ProjectAdvisor
        Run Keyword And Return Status    Input Text    //input[@id='projectAdvisor']    ${ProjectAdvisor}
...
Handle Submission Result
    ...
    ELSE
        ...
        sleep    2
        Capture Page Screenshot    Project_Test_AcademicService/TC11_EditProposal/Screenshots_AlertNotFound/${i}_AlertNotFound.png
...
Validate And Write Result
    ...
    IF    ${compare_result}
        ...
        Capture Page Screenshot    Project_Test_AcademicService/TC11_EditProposal/Screenshots_Pass/${i}_${ActualMessage}.png
        Go To    ${URL}
    ELSE
        Write Excel Cell    ${i}    23    FAIL
        Capture Page Screenshot    Project_Test_AcademicService/TC11_EditProposal/Screenshots_Fail/${i}_${ActualMessage}.png
        Go To    ${URL}
    END
